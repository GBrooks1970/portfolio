...
class GenerateSiteTests(unittest.TestCase):
    ...
    def test_renders_current_public_inventory_and_counts(self) -> None:
        self.assertEqual(self.rendered.count('<article class="card" data-project='), 11)
        self.assertIn('data-project="portfolio-prompts"', self.rendered)
        self.assertIn("Eleven showcase projects", self.rendered)
        self.assertIn("All eleven showcase project repositories", self.rendered)
        self.assertIn("ParaBank Bank Automation", self.rendered)
        self.assertIn("Auth Separation Screenplay POC", self.rendered)
    ...
    def test_action_types_are_rendered_with_distinct_classes(self) -> None:
        ...
        self.assertIn(
            '<a class="btn documentation" '
            'href="https://gbrooks1970.github.io/auth-separation-screenplay-poc/">API reference</a>',
            self.rendered,
        )
        self.assertIn(
            '<a class="btn report" '
